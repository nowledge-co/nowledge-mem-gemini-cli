...
- Clarified remote MCP setup: durable remote credentials still live in the shared `nmem` client config, while Gemini's MCP transport needs a host-owned `mcpServers.nowledge-mem` override generated with `nmem config mcp show --host gemini-cli`.

## [0.1.7] - 2026-04-27

### Added

- Added a bundled local Nowledge Mem MCP server so Gemini can call retrieval and memory-write tools directly in addition to the existing `nmem` commands and lifecycle hooks. User `settings.json` MCP configuration with the same server name still takes precedence for remote Mem or custom endpoints.

## [0.1.6] - 2026-04-27

### Added

- Added a `PreCompress` lifecycle hook so Gemini imports the current thread before context compression, not only when the session exits.

## [0.1.5] - 2026-04-14

### Changed

- Working Memory guidance now uses space wording that matches how users actually think about lanes, instead of internal ID-style language
- Search and Working Memory prompts now explain more clearly when Gemini should stay in the current lane and when it should follow shared-space retrieval configured in Mem
- README now matches the same space-aware guidance, so install, prompt behavior, and troubleshooting tell one coherent story
